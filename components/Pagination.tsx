...
export default function Pagination({
  ...
  return (
    <div className="w-full flex justify-between mt-8 items-center gap-2">
      <p className="text-[20px] leading-[30px] font-medium text-[#2A2A2A] w-full lg:block hidden">
        <span className="font-semibold">{(currentPage - 1) * perPage + 1}</span>{" "}
        to{" "}
        <span className="font-semibold">
          {Math.min(currentPage * perPage, totalItems)}
        </span>{" "}
        of <span className="font-semibold">{totalItems}</span> results shown
      </p>
